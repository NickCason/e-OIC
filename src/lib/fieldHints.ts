...
const PHASE_OPTS: readonly string[] = ['Single', 'Three', 'N/A'];
const PROTOCOL_OPTS: readonly string[] = [
    'EtherNet/IP', 'Modbus TCP', 'Modbus RTU', 'DeviceNet', 'ControlNet',
    'Profibus', 'Serial', 'RIO', 'Fieldbus', 'N/A',
];
const VOLTAGE_OPTS: readonly string[] = [
    '24 VDC', '12 VDC', '120 VAC', '208 VAC', '240 VAC', '277 VAC',
    '480 VAC', '600 VAC', 'N/A',
];

export const ENUM_HEADERS: Record<string, readonly string[]> = {
    Phase: PHASE_OPTS,
    'Phase In': PHASE_OPTS,
...
export function getHint(header: string): string | null {
    return HINTS[header] || null;
}

export function getEnumOptions(header: string): readonly string[] | null {
    return ENUM_HEADERS[header] || null;
}

export function isSharedHeader(header: string): boolean {
    return SHARED_HEADERS.has(header);
}

// Stable, datalist-id-safe slug from a column header.
export function slugForId(header: string): string {
