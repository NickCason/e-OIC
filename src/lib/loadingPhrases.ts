...
export type PhraseSetName =
    | 'general' | 'export' | 'parse' | 'diff' | 'apply' | 'build' | 'photo';

export const PHRASES: Record<PhraseSetName, readonly string[]> = {
...
export function pickPhrase(
    setName: PhraseSetName | string = 'general',
    exclude: readonly string[] = [],
): string {
    const list: readonly string[] = (PHRASES as Record<string, readonly string[]>)[setName]
        ?? PHRASES.general;
    const pool = list.filter((p) => !exclude.includes(p));
    const final = pool.length ? pool : list;
    // noUncheckedIndexedAccess: list/final are non-empty here (PHRASES.general
    // is non-empty), so the index access is safe — assert with `??`.
    return final[Math.floor(Math.random() * final.length)] ?? '';
}

// Back-compat: existing imports of LOADING_PHRASES should still work.
export const LOADING_PHRASES: readonly string[] = PHRASES.general;
