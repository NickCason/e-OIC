// seed.ts — load the bundled sample job into IndexedDB on first launch.
...
import type { IBackupSnapshot, IImportStats } from '../db';

const FLAG = 'sampleSeeded';

async function fetchSeed(): Promise<IBackupSnapshot> {
    const resp = await fetch('./seed.json');
    if (!resp.ok) throw new Error(`seed.json ${resp.status}`);
    return resp.json() as Promise<IBackupSnapshot>;
}

export async function maybeSeedSampleJob(): Promise<boolean> {
    if (await getSetting<boolean>(FLAG)) return false;
    try {
...
export async function reloadSampleJob(): Promise<IImportStats> {
    const snapshot = await fetchSeed();
